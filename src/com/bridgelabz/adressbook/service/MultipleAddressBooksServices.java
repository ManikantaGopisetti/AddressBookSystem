...
public class MultipleAddressBooksServices {
	...
	public void viewPersonByCityState() {
		...
		case 1:
			...
			System.out.println("Enter name of the city");
			String name = sc.next();
			cityDictionary.entrySet().stream().filter(contact->contact.getKey().equals(name)).forEach(contact->System.out.println(contact.getValue()));;			
			break;
		...
		case 2:
			...
			System.out.println("Enter name of the state");
			name = sc.next();
			stateDictionary.entrySet().stream().filter(contact->contact.getKey().equals(name)).forEach(contact->System.out.println(contact.getValue()));;
